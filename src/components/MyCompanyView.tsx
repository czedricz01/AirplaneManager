...
function Delta({ current, previous }: { current: number; previous?: number }) {
  if (previous === undefined) {
    return <span className="text-[10px] font-mono text-white/30 uppercase tracking-widest">First month</span>;
  }
  ...
  return (
    <span className={`flex items-center gap-1 text-[10px] font-mono ${up ? 'text-aero-good' : 'text-aero-warn'}`}>
      <Icon size={10} />
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between mb-2">
        <span className="text-[10px] uppercase tracking-widest text-white/40 font-black">{title}</span>
        ...
        {reports.map((r, i) => {
          ...
          return (
            <div key={`${r.year}-${r.month}`} className="relative flex-1 group" title={`${label(r)}: ${formatCurrency(v)}`}>
              ...
              />
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white text-black text-[9px] px-1.5 py-0.5 whitespace-nowrap z-10 font-black pointer-events-none">
        ...
        <div className="grid grid-cols-1 gap-3 max-w-4xl mx-auto pb-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="bg-black/40 border border-white/10 p-4 rounded-sm">
              <span className="text-[10px] uppercase tracking-widest text-white/40 font-black mb-2 block">Cash</span>
              <span className="text-2xl font-mono font-bold text-aero-yellow">{formatCurrency(capital)}</span>
            </div>
            <div className="bg-black/40 border border-white/10 p-4 rounded-sm">
              <span className="text-[10px] uppercase tracking-widest text-white/40 font-black mb-2 block">Fleet value</span>
              <span className="text-2xl font-mono font-bold text-white/80">{formatCurrency(fleetValue)}</span>
              <span className="block text-[9px] font-mono text-white/30 mt-1">{fleetCount} aircraft, at resale</span>
            </div>
            <div className="bg-black/40 border border-white/10 p-4 rounded-sm">
            ...
            <div className="bg-black/40 border border-white/10 p-4 rounded-sm">
              ...
              <span className="block text-[9px] font-mono text-white/30 mt-1">active routes</span>
            </div>
          </div>

          {reportHistory.length === 0 ? (
            <div className="h-48 border border-white/5 bg-black/20 flex flex-col items-center justify-center gap-2 text-white/40 uppercase tracking-widest text-xs font-bold">
              No month closed yet
              <span className="text-[10px] normal-case tracking-normal text-white/30 font-mono">
            ...
            <>
              ...
              <div className="bg-black/40 border border-white/10 p-4 rounded-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <span className="text-[10px] uppercase tracking-widest text-white/40 block mb-1">Revenue</span>
                  ...
                  <div>
                    <span className="text-[10px] uppercase tracking-widest text-white/40 block mb-1">Operating profit</span>
                  ...
                  <div>
                    <span className="text-[10px] uppercase tracking-widest text-white/40 block mb-1">Cash change</span>
              ...
              <div className="bg-black/40 border border-white/10 p-4 rounded-sm">
                ...
                <p className="text-[10px] font-mono text-white/30 mt-3 leading-relaxed">
              ...
              <div className="p-4 bg-white/5 border border-white/10 rounded-sm flex gap-4">
                ...
                <div className="text-[11px] text-white/60 leading-relaxed font-mono space-y-1.5">
